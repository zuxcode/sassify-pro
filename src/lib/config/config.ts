...
class Config {
  private packageName = 'sassify-pro';
  ...
  readPackageJson() {
    ...
    exec('npm root', (error, stdout) => {
      ...
      exec(`ls ${this.packagePath}`, (err) => {
        if (!err) {
          console.log(
            `The package '${this.packageName}' is installed locally at: ${this.packagePath}`,
          );
        ...
        console.log(
          `The package '${this.packageName}' is not installed locally.`,
        );
        ...
        exec('npm root -g', (er, print) => {
          ...
          exec(`ls ${this.packagePath}`, (e) => {
            if (!e) {
              console.log(
                `The package '${this.packageName}' is installed globally at: ${this.packagePath}`,
              );
              this.packageJson = JSON.parse(
                fs.readFileSync(this.packagePath, 'utf8'),
              );
            ...
            console.log(
              `The package '${this.packageName}' is not installed globally.`,
            );
            return 1;
          });
          return 1;
