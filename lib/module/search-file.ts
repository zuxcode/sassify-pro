...
class SearchFile {
  ...
  public static searchFile(sourcePath: string, excludePath: string[]) {
    const matchWhiteSpaceEmptyString = /^$|\s+/;

    const childStackDirectoryDependency: string[] = [];
    ...
    const isInvalidPath = (filePath: string) => matchWhiteSpaceEmptyString.test(filePath);

    if (isInvalidPath(sourcePath)) {
      throw new Error(
        'Invalid file. File cannot contain white space or empty string',
      );
    }

    const stack: string[] = fs.readdirSync(sourcePath);

    stack.forEach((currentStack: string) => {
      ...
      exclude.forEach((childStackFile) => {
        ...
        if (stackFullPathStat.isDirectory()) {
          const childStackDirectory: string[] = fs.readdirSync(childStackFile);
